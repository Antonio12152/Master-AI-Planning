...
class PlanService
{
     ...
     * Создать новый план
     */
    public function createPlan(array $data, $user): Plan
    {
        // Создать план
     ...
     * Обновить план
     */
    public function updatePlan(Plan $plan, array $data, $user): Plan
    {
     ...
     */
    public function deletePlan(Plan $plan, $user): bool
    {
     ...
     */
    public function getUserAccessiblePlans($user, int $perPage = 15): Paginator
    {
     ...
     */
    public function getUserCreatedPlans($user, int $perPage = 15): Paginator
    {
        return $user->plans()
            ->with(['ideaGroups', 'members.user'])
            ->orderBy('created_at', 'desc')
     ...
     */
    public function getUserSharedPlans($user, int $perPage = 15): Paginator
    {
     ...
     * Архивировать план
     */
    public function archivePlan(Plan $plan, $user): Plan
    {
        $plan->update([
            'status' => 'archived',
            'archived_at' => now(),
     ...
     */
    public function unarchivePlan(Plan $plan, $user): Plan
    {
        $plan->update([
            'status' => 'active',
            'archived_at' => null,
     ...
     * Логировать действие
     */
    private function logAction($user, Plan $plan, string $action, array $extra = []): void
    {
